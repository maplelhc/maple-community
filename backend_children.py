...
def init_db():
    with get_db_connection() as conn:
        ...
        # ---------- 儿童节红包列 ---------- (仍然保留列，但红包函数不再使用)
        if 'last_redpacket_date' not in existing_columns:
...
# ---------- 前端入口 ----------
@app.route('/maple_children.html')
def serve_frontend():
    return send_from_directory('.', 'maple_children.html')
